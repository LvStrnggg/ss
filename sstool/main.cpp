...
std::vector<void*> pattern_scan(HANDLE hProcess, const std::vector<std::string_view>& patterns) {
    ...
    uint8_t* address = (uint8_t*) sys_info.lpMinimumApplicationAddress;
    ...
    while (address < sys_info.lpMaximumApplicationAddress && VirtualQueryEx(hProcess, address, &memInfo, sizeof(memInfo))) {
        ...
        if (memInfo.State == MEM_COMMIT && (memInfo.Protect & PAGE_READWRITE || memInfo.Protect & PAGE_EXECUTE_READWRITE) &&
            ...
            if (ReadProcessMemory(hProcess, memInfo.BaseAddress, buffer.data(), buffer.size(), &bytesRead)) {
                std::string_view view((char*) buffer.data(), bytesRead);

                for (const std::string_view& pattern : patterns) {
                    for (size_t pos = view.find(pattern); pos != std::string_view::npos; pos = view.find(pattern, pos + 1)) {
                        void* found = (uint8_t*) memInfo.BaseAddress + pos;

                        std::cout << "[*] Found string " << pattern << " at " << std::hex << std::uppercase << (uintptr_t) found << "\n";
        ...
        address = (uint8_t*) memInfo.BaseAddress + memInfo.RegionSize;
    }
    return results;
}